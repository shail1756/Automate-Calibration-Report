...
if df_filtered.empty:
    st.info("No records found for the selected date window. Adjust the dates to generate a summary.")
else:
    gen_excel = st.button("📑 Generate Excel Summary (Current Window)")
    ...
    if gen_excel:
        ...
        for idx, row in df_filtered.iterrows():
            ...
            master_rows = master_instrument_list[
                master_instrument_list["Serial No."].astype(str).str.upper() == master_serial.upper()
            ]

            # Skip if instrument or master mapping is missing
            if inst_rows.empty or master_rows.empty:
                continue
        ...
        if not summary_records:
            st.warning("No valid records to include in the summary (missing instrument/master mapping).")
        else:
